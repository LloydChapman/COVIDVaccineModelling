...
source("Code/prediction_functions.R")
source("Code/calc_hosp_ICU_death_risk.R")

fnms <- list.files("Data/",pattern = "death_regression_output_.*_1.RDS")
...
r <- 1^(1/t_sim) #1.1^(1/t_sim) # per day multiplier for force of infection = 10% increase per month, currently
...
# CFH <- read.csv("Data/COVIDForecastHubData/2020-09-21-all-forecasted-cases-model-data.csv",stringsAsFactors = F)
# CFH <- read.csv("Data/COVIDForecastHubData/2020-10-19-all-forecasted-cases-model-data.csv",stringsAsFactors = F)
CFH <- read.csv("Data/COVIDForecastHubData/2020-11-23-all-forecasted-cases-model-data.csv",stringsAsFactors = F)
...
CalCAT <- read.csv("Data/CalCATdata/CalCAT_Custom_Data_2020-12-11.csv",stringsAsFactors = F)
View(CalCAT)
...
for (i in 1:length(fnms)){
  ...
  x <- predict_deaths(paste0("Data/",fnms[i]),t_sim,r,deaths_ratio[i])
